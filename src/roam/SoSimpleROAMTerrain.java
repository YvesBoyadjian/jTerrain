...
static boolean first_run = true;
  ...
  if (first_run)
  {
    ...
    triangle_tree = new Array<SbROAMTriangle>(SbROAMTriangle.class,tree_size);
  ...
  ******************************************************************************/
    ...
    if (parent.level < level)
    {
      ...
      parent.error = Math.max(left_child.error, right_child.error) +
        Math.abs(apex.getValue()[2] - (first.getValue()[2] + second.getValue()[2]) * 0.5f);
  ...
  protected final Array<SbPlane> planes = new Array<>(SbPlane.class,6);
    /* Datove polozky. */
