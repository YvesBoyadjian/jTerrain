  ...
  final SbVec3f center)
{
  ...
  else
  {
    ...
    max.getValue()[2] = (max.getValue()[1] - min.getValue()[1]) * 0.5f;
    min.getValue()[2] = -max.getValue()[2];
  ...
  int index, SbBox2s coord_box)
{
  ...
  if (((index << 2) + 4) >= this.tile_tree.tree_size)
  {
    ...
    // Full detailed tile.
    tile.error = (tile.bounds.getMax().getValue()[2] - tile.bounds.getMax().getValue()[2]) * 0.01f;
  }
  else
  {
      ...
      for (int X = min_x; X <= max_x; X+= inc_x, ++vertex_index)
      {
        ...
        if ((Y < max_y) && (X < max_x))
        {
          ...
          final SbVec3f half_h_vertex = this.coords[this_index +
            half_inc_x];
          float tmp_error = SbBasic.SbAbs(half_h_vertex.getValue()[2] - ((this_vertex.getValue()[2] +
            next_h_vertex.getValue()[2]) * 0.5f));
          ...
          final SbVec3f half_v_vertex = this.coords[this_index + (half_inc_y
            * this.map_size)];
          tmp_error = SbBasic.SbAbs(half_v_vertex.getValue()[2] - ((this_vertex.getValue()[2] + next_v_vertex.getValue()[2])
            * 0.5f));
          max_error = SbBasic.SbMax(max_error, tmp_error);
          ...
          tmp_error = SbBasic.SbAbs(half_d_vertex.getValue()[2] - ((this_vertex.getValue()[2] + next_d_vertex.getValue()[2])
            * 0.5f));
          max_error = SbBasic.SbMax(max_error, tmp_error);
        }
  ...
  float morph, int center_index, int first_index, int second_index)
{
  ...
  final SbVec3f tmp_vertex = new SbVec3f(coords[center_index].getValue());
  final SbVec3f first_vertex = new SbVec3f(coords[first_index].getValue());
  final SbVec3f second_vertex = new SbVec3f(coords[second_index].getValue());
  tmp_vertex.getValue()[2] = (tmp_vertex.getValue()[2] * morph) + ((first_vertex.getValue()[2] +
    second_vertex.getValue()[2]) * 0.5f * (1.0f - morph));
  gl2.glVertex3fv(tmp_vertex.getValue(),0);
}
  ...
  final SbChunkedLoDTile tile)
{
  ...
  float skirt_height = (tile.bounds.getMax().operator_minus( tile.bounds.getMin())).getValue()[2] * 0.2f;
  ...
  for (int X = 0; X < max_x; ++X)
  {
    ...
    second_vertex.getValue()[2]-= skirt_height;
    ...
    gl2.glVertex3fv(first_vertex.getValue(),0);
    gl2.glVertex3fv(second_vertex.getValue(),0);
  }
  gl2.glEnd();
  ...
  final SbChunkedLoDTile tile)
{
  ...
  for (int Y = 0; Y < (max_y - 1); ++Y)
  {
    ...
    for (int X = 0; X < max_x; ++X)
    {
      ...
      if (this.is_normals)
      {
        gl2.glNormal3fv(this.normals[bottom_index].getValue(),0);
      }
      gl2.glVertex3fv(this.coords[bottom_index].getValue(),0);
      ...
      if (this.is_normals)
      {
        gl2.glNormal3fv(this.normals[top_index].getValue(),0);
      }
      gl2.glVertex3fv(this.coords[top_index].getValue(),0);
    }
    gl2.glEnd();
  }
}
