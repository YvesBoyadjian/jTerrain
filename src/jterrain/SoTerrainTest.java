...
public class SoTerrainTest {
	...
	static void styleCallback(Object userData, SoEventCallback eventCB)
	{
	  ...
	  static final Array<SbVec3f> camera_positions = new Array<>(SbVec3f.class,
	  ...
	  static final Array<SbRotation> camera_orientations = new Array<>(SbRotation.class,
	...
	public static void main(String argv[])
	{
	  ...
	  byte[] heightmap = simage_read_image(heightmap_name, width,
	    height, components);
	  if (heightmap == null)
	  {
	  ...
	  for (int I = 0; I < width[0] * height[0]; ++I)
	  {
	    ...
	    int hm = heightmap[/*I*/index]; if(hm<0)hm+=256;
	  ...
	  switch (algorithm)
	  {
	    ...
	    case ID_ALG_BRUAL_FORCE:
	    {
	      ...
	      Integer[] indices = terrain.coordIndex.startEditing();
	...
	private static void simage_free_image(byte[] heightmap) {
		// TODO Auto-generated method stub
		
	}

	private static byte[] simage_read_image(String heightmap_name, int[] width, int[] height, int[] components) {
		...
		try {
			...
			byte imBytes[] = buffer.getData().clone();
			return imBytes;
		} catch (IOException e) {
