  ...
  final SbBox2s coord_box)
{
  ...
  tile.levels = new Array<>(SbGeoMipmapTileLevel.class,tile_tree.level_count);
    ...
    for (int X = 0; X < parent_size; ++X)
    {
      ...
      if ((X & 0x01)!=0 || (Y & 0x01)!=0)
      {
        ...
        if ((Y == 0) || (Y == (parent_size - 1)))
        {
          ...
          float tmp_error = SbBasic.SbAbs(coords[parent.vertices[index]].getValue()[2] -
            ((coords[parent.vertices[index - 1]].getValue()[2] +
            coords[parent.vertices[index + 1]].getValue()[2]) * 0.5f));
        ...
        else if ((X == 0) || (X == (parent_size - 1)))
        {
          int index = (Y * parent_size) + X;
          float tmp_error = SbBasic.SbAbs(coords[parent.vertices[index]].getValue()[2] -
            ((coords[parent.vertices[index - parent_size]].getValue()[2] +
            coords[parent.vertices[index + parent_size]].getValue()[2]) * 0.5f));
          max_error = SbBasic.SbMax(max_error, tmp_error);
        }
        ...
        else
        {
          int index = (Y * parent_size) + X;
          float tmp_error = SbBasic.SbAbs(coords[parent.vertices[index]].getValue()[2] -
            ((coords[parent.vertices[index - parent_size + 1]].getValue()[2] +
            coords[parent.vertices[index + parent_size - 1]].getValue()[2]) * 0.5f));
          max_error = SbBasic.SbMax(max_error, tmp_error);
        }
      }
...
public final void GL_SEND_VERTEX(GL2 gl2, int index) { int vertex_index = index; 
  ...
  if (is_normals) 
    gl2.glNormal3fv(normals[vertex_index].getValue(),0); 
  gl2.glVertex3fv(coords[vertex_index].getValue(),0);
}
